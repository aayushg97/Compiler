...
%type <val_int> paramlist paramtypelist
...
primexp: ID			{	
	...
	| FLOAT_CONST	{	
						...
						($$.addr)->type->numelem = global_symtab->strcount;
						($$.addr)->type->isarr = 0;								// set the type
						...
						char lexeme[10];
						sprintf(lexeme,".LC%d",global_symtab->strcount);		// generating a temporary
						entrySym *use = global_symtab->lookup(lexeme);
						use->initval = str1;
						use->type = new mtype();							//
						use->type->ntype = "fconst";						//
						use->type->numelem = global_symtab->strcount;
						use->type->isarr = 0;								// set the type
						use->type->width = double_width;					//
						use->type->next = NULL;
						use->size = ($$.type)->width;
						global_symtab->offset += ($$.addr)->size;
						global_symtab->strcount += 1;
					}
	...
	| SLIT			{
						$$.addr = tableSym::gentemp();							// generate temporary
						($$.addr)->type = new mtype();							//
						($$.addr)->type->ntype = "string";						//
						($$.addr)->type->isarr = 0;								// set the type
						($$.addr)->type->numelem = global_symtab->strcount;
						($$.addr)->type->width = char_width*strlen($1);			//
						($$.addr)->type->next = NULL;							//
						($$.addr)->initval = $1;
						($$.addr)->size = ($$.addr)->type->width;				// set size
						curr_symtab->offset += ($$.addr)->size;					// update total offset of the symbol table
...
postexp:    primexp					{
	...
	| postexp DOTAPOS				{	
										...
										entrySym *t = tableSym::gentemp();
										t->type = ($$.type)->next->next;
										t->size = t->type->width;
										curr_symtab->offset += t->size;
										
										for(int i=0;i<($$.type)->numelem;i++){
											for(int j=0;j<($$.type)->next->numelem;j++){
												stringstream s1,s2;
												s1 << ((j*(($$.type)->numelem))+i)*(($$.type)->next->next->width);
												s2 << ((i*(($1.type)->numelem))+j)*(($1.type)->next->next->width);
												quadArray::emit("rindexed",($1.array)->name,s1.str(),t->name);
												quadArray::emit("lindexed",s2.str(),t->name,($$.array)->name);
											}
										}
	...
	| unaryop castexp				{	
										switch($1){
											...
											case '&': 
												...
												($$.addr)->type->width = point_width;				//
												($$.addr)->type->isarr = 0;						//
												($$.addr)->size = point_width;					// set size of temporary in symbol table
												curr_symtab->offset += ($$.addr)->size;			// update total offset of symbol table
...
multexp:    castexp					{$$ = $1;}
	...
	| multexp '*' castexp			{
										...
										if($1.matop==1 && $3.matop==1){
											...
											entrySym *t1 = tableSym::gentemp();
											t1->type = ($$.type)->next->next;
											t1->size = t1->type->width;
											curr_symtab->offset += t1->size;
											
											entrySym *t2 = tableSym::gentemp();
											t2->type = ($$.type)->next->next;
											t2->size = t2->type->width;
											curr_symtab->offset += t2->size;
											
											// emit quad of matrix multiplication instruction 
											for(int i=0;i<($1.type)->numelem;i++){
												for(int j=0;j<($3.type)->next->numelem;j++){
													for(int k=0;k<($3.type)->numelem;k++){
														stringstream s1,s2,s3;
														s1 << ((i*(($1.type)->next->numelem))+k)*(($1.type)->next->next->width);
														s2 << ((k*(($3.type)->next->numelem))+j)*(($3.type)->next->next->width);
														s3 << ((i*(($3.type)->next->numelem))+j)*(($3.type)->next->next->width);
														quadArray::emit("rindexed",($1.array)->name,s1.str(),t1->name);
														quadArray::emit("rindexed",($3.array)->name,s2.str(),t2->name);
														quadArray::emit("*",t1->name,t2->name,t1->name);
														quadArray::emit("lindexed",s3.str(),t1->name,($$.array)->name);
													}
												}
											}
...
addexp:    multexp					{$$ = $1;}
	...
	| addexp '+' multexp			{
										...
										if($1.matop==1 && $3.matop==1){
											...
											for(int i=0;i<($1.type)->numelem;i++){
												for(int j=0;j<($1.type)->next->numelem;j++){
													stringstream s;
													s << ((i*(($1.type)->next->numelem))+j)*(($1.type)->next->next->width);
													quadArray::emit("rindexed",($1.array)->name,s.str(),t1->name);
													quadArray::emit("rindexed",($3.array)->name,s.str(),t2->name);
													quadArray::emit("+",t1->name,t2->name,t1->name);
													quadArray::emit("lindexed",s.str(),t1->name,($$.array)->name);
												}
											}
											//quadArray::emit("+",($1.array)->name,($3.array)->name,($$.array)->name);
										}
	...
	| logorexp '?' exp ':' condexp	/* if logorexp (i.e. $1) is true jump to the code [t = exp (i.e. $3)] and go to the end of condexp(i.e. $$) else jump to the code [t = condexp (i.e. $5)].*/
										{
											...
											if($3.matop==1 && $5.matop==1){
												...
												entrySym *t = tableSym::gentemp();
												t->type = ($3.type)->next->next;
												t->size = t->type->width;
												curr_symtab->offset += t->size;
												
												// emit quad of matrix copy instruction
												for(int i=0;i<($3.type)->numelem;i++){
													for(int j=0;j<($3.type)->next->numelem;j++){
														stringstream s;
														s << ((i*(($3.type)->next->numelem))+j)*(($3.type)->next->next->width);
														quadArray::emit("rindexed",($3.array)->name,s.str(),t->name);
														quadArray::emit("lindexed",s.str(),t->name,($$.array)->name);
													}
												}
											...
											if($3.matop==1 && $5.matop==1){
												...
												entrySym *t = tableSym::gentemp();
												t->type = ($5.type)->next->next;
												t->size = t->type->width;
												curr_symtab->offset += t->size;
											
												// emit quad of matrix copy instruction
												for(int i=0;i<($5.type)->numelem;i++){
													for(int j=0;j<($5.type)->next->numelem;j++){
														stringstream s;
														s << ((i*(($5.type)->next->numelem))+j)*(($5.type)->next->next->width);
														quadArray::emit("rindexed",($5.array)->name,s.str(),t->name);
														quadArray::emit("lindexed",s.str(),t->name,($$.array)->name);
													}
												}
												...
												if($1.matop==1 && $3.matop==1){
													...
													for(int i=0;i<($3.type)->numelem;i++){
														for(int j=0;j<($3.type)->next->numelem;j++){
															...
															s << ((i*(($3.type)->next->numelem))+j)*(($3.type)->next->next->width);
															quadArray::emit("rindexed",($3.array)->name,s.str(),t->name);
															quadArray::emit("lindexed",s.str(),t->name,($1.array)->name);
														}
													}
													//quadArray::emit(($3.array)->name,($1.array)->name);
												}
												// emit quad of left indexed copy instruction
												else quadArray::emit("lindexed",($1.addr)->name,($3.addr)->name,($1.array)->name);
											}
											else{
												if(($3.type)->ntype!="string")typecheck(&$1,&$3);
												// emit quad of copy instruction
	...
	| dector '=' initializer				// Non-terminal Q sets a flag that prevents generation of quads for initialization of a variable
											{
												if(($1.type)->isarr==0){
													...
													if($3.express.matop==1){
														entrySym *t = tableSym::gentemp();
														t->type = ($3.express.type)->next->next;
														t->size = t->type->width;
														curr_symtab->offset += t->size;
											
														// emit quad of matrix copy instruction
														for(int i=0;i<($3.express.type)->numelem;i++){
															for(int j=0;j<($3.express.type)->next->numelem;j++){
																stringstream s;
																s << ((i*(($3.express.type)->next->numelem))+j)*(($3.express.type)->next->next->width);
																quadArray::emit("rindexed",($3.express.array)->name,s.str(),t->name);
																quadArray::emit("lindexed",s.str(),t->name,($1.array)->name);
															}
														}
														//quadArray::emit(($3.express.array)->name,($1.addr)->name);
													}
													else{
														int col = (($3.iarr1)->operator[](0))->size();
														for(int i=0;i<($3.iarr1)->size();i++){
															for(int j=0;j<col;j++){
																stringstream ss;
																ss << ((i*col)+j)*(($1.type)->next->next->width);
																quadArray::emit("lindexed",ss.str(),((($3.iarr1)->operator[](i))->operator[](j))->name,($1.array)->name);
															}
														}
													}
												}
...
paramlist:	paramdection			{$$ = 1;}
	| paramlist ',' paramdection		{$$ = $1 + 1;}
	;
...
initializer:	assignexp			{
	...
	| '{' initializerrowlist '}'	{
										//$$.express = $2.express;
										$$.iarr1 = new vector< vector <entrySym*> *>();
										for(int i = 0;i<($2.iarr1)->size();i++){
											($$.iarr1)->push_back(($2.iarr1)->operator[](i));
										}
...
initializerrowlist:	initializerrow				{
													...
													//$$.express = $1.express;
													($$.iarr1) = new vector< vector<entrySym*> *>();
													($$.iarr1)->push_back($1.iarr2);
												}
	| initializerrowlist ';' initializerrow		{
													$$.val = new string((*($1.val)) + ";" + (*($3.val)));	// saving the initial value
													//$$.express = $1.express;	
													($$.iarr1) = new vector< vector<entrySym*> *>();
													for(int i = 0;i<($1.iarr1)->size();i++){
														($$.iarr1)->push_back(($1.iarr1)->operator[](i));
													}
													($$.iarr1)->push_back($3.iarr2);
												}
	;
...
initializerrow:	initializer						{
													$$.val = new string(*($1.val));			// saving the initial value
													//$$.express = $1.express;
													($$.iarr2) = new vector<entrySym*>();
													($$.iarr2)->push_back(($1.express).addr);
												}
	...
	| initializerrow ',' initializer			{
													...
													//$$.express = $1.express;
													($$.iarr2) = new vector<entrySym*>();
													for(int i = 0;i<($1.iarr2)->size();i++){
														($$.iarr2)->push_back(($1.iarr2)->operator[](i));
													}
													($$.iarr2)->push_back(($3.express).addr);
												}
	;
